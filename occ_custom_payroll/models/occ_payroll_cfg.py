...
def get_attendance_type(
    ...
    holiday_status = get_holiday_status(self, date_now, work_location)
...
def get_attendance_status(
    ...
    holiday_status = get_holiday_status(self, date, work_location)
    if holiday_status.get("count") > 0:
        value = "7"
...
def get_attendance_actual(self):
    ...
    if attendance_obj:
        ...
        if attendance_obj[0][1]:
            ...
            in_hr = int(obj_check_in.astimezone(tz).strftime("%-H"))
            in_min = int(obj_check_in.astimezone(tz).strftime("%-M"))

            actual_in_dt = obj_check_in.astimezone(tz)
        ...
        if attendance_obj[0][2]:
            ...
            out_hr = int(obj_check_out.astimezone(tz).strftime("%-H"))
            out_min = int(obj_check_out.astimezone(tz).strftime("%-M"))

            actual_out_dt = obj_check_out.astimezone(tz)
...
def _ot_mail_send(
    ...
    if approver == 1:
        ...
        template = self.env.ref(
            "occ_payroll_config.email_overtime_supervisor_notification_template"
        )
        ...
        template = self.env.ref(
            "occ_payroll_config.email_overtime_manager_notification_template"
        )
